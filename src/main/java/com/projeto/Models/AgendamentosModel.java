...
public class AgendamentosModel {
    ...
    public BigDecimal getValorLiquidoTotal() {
        ...
        if (valorParcial != null && valorParcial.compareTo(BigDecimal.ZERO) > 0) {
            ...
            // Fallback para dados antigos (se snapshot for nulo, tenta pegar do cadastro atual)
            if (taxaUsar == null && formaPagamentoSinal != null) taxaUsar = formaPagamentoSinal.getTaxa();
            if (repassarUsar == null && formaPagamentoSinal != null) repassarUsar = formaPagamentoSinal.getRepassarTaxa();
            
            // Garante valores seguros
            if (taxaUsar == null) taxaUsar = BigDecimal.ZERO;
            if (repassarUsar == null) repassarUsar = false;

            // Executa a conta
            if (repassarUsar) {
                // Se repassou, o valorParcial já inclui a taxa, mas o líquido é o valorParcial original (ou seja, entra cheio)
                // *Obs: Na lógica de repasse, o cliente paga X + Taxa. O que cai na conta é o X.
        ...
        if (pagamentos != null) {
            for (PagamentosModel pg : pagamentos) {
                if (pg.getValor() != null) {
                    ...
                    // Fallback
                    if (taxaPg == null && pg.getFormaPagamento() != null) taxaPg = pg.getFormaPagamento().getTaxa();
                    if (repassaPg == null && pg.getFormaPagamento() != null) repassaPg = pg.getFormaPagamento().getRepassarTaxa();

                    if (taxaPg == null) taxaPg = BigDecimal.ZERO;
                    if (repassaPg == null) repassaPg = false;
