...
public class AgendamentosController {
    ...
    public AgendamentosModel criarAgendamento(@RequestBody AgendamentosModel agendamento) {
        ...
        // LÓGICA DE VÍNCULO:
        // Precisamos dizer a cada pagamento que ele pertence a ESTE agendamento
        if (agendamento.getPagamentos() != null) {
            for (PagamentosModel pg : agendamento.getPagamentos()) {
                pg.setAgendamento(agendamento); // <--- Isso preenche o agendamento_id
            }
        }
    ...
    // --- ATUALIZAR (PUT) ---
    @PutMapping("/{id}")
    public ResponseEntity<AgendamentosModel> atualizarAgendamento(@PathVariable Long id, @RequestBody AgendamentosModel novosDados) {
